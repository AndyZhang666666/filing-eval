...
export function scoreRun({ items, rubricVersion, model, promptVersion }) {
  ...
  for (const item of items) {
    ...
    if (errItem && !omission) {
      ...
      if (expect.unsupported === "high") {
        checks++;
        const ok = u >= 3;
        hit = hit && ok;
        det.push(`无据 期望大量 实际${u}${ok ? " ✓" : " ✗"}`);
      }
    }
    ...
    const falsePositive = faithful && (c > 0 || u > 0);
    ...
    if (errItem && !omission && expect.min_coverage != null) det.push(`（覆盖 ${judge.coverage_rate} 仅记录）`);
